...
//<p id="ref">Acknowledgement: "National Consortium for the Study of Terrorism and Responses to Terrorism (START). (2016). 
//Global Terrorism Database [Data file]. <br/> Retrieved from <a href="https://www.kaggle.com/START-UMD/gtd">https://www.kaggle.com/START-UMD/gtd</a> "</p>
d3.select("acontent").append("p")
    .attr("id", "ref")
...
// Ordinal color scale
var fill = d3.scale.ordinal().domain(["No", "Yes"]).range([ "#6CA9D6", "#F29319"]);
...
var legendTitle = svgContainer.append("g")
                           .attr("transform", "translate(" + (width-360) +","+ (height-670) + ")")
                          .append("text")
...
var legend = svgContainer.append("g")
            .attr("transform", "translate(" + (width - 350) + "," + (height - 660) + ")" )
              .selectAll("g")
              .data(fill.domain())
              .enter()
            .append("g")
...
d3.json("data/us-states.json", function (error, json) {
      ...
      mapGroup.append("path").attr("d", geoPath(json))
                    .style("fill", "#d9d9d9")
                    .style("stroke", "#fff")
      ...
      function plot_points( data ) {
              ...
              var radius = d3.scale.sqrt( ).domain( [ 0, totalVictims_max ] ).range( [ 5, 20 ] );
              ...
              var myslider =  chroniton()
                    ...
                    .on("change", function(date) {
                       ...
                       if (newYear != currentYear) { 
                              ...
                              mapGroup.selectAll('circle').on("mouseover", function(d) {
                                  tip.show(d);
